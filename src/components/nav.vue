<template>
    <div class="flex justify-center w-full h-[50px] bg-transparent text-[var(--mainColor)] bgglass">
        <nav class="flex justify-between items-center h-full w-[90%]">
            <!-- Mobile menu button -->
            <div class="meniu flex-col h-full w-[30px] justify-center gap-2 hidden cursor-pointer" @click="open = !open"
                v-tooltip="'Menu'">
            ...
            <!-- Logo -->
            <div class="font-bold text-xl logo w-[30%] flex justify-center items-center gap-1"
                v-tooltip="'Mohammad Falah - Portfolio'">
                MOHAMMAD FALAH
            </div>

            <!-- Navigation links -->
            <ul class="font-bold flex justify-evenly w-[50%] pages" :class="currentLanguage == 'ar' ? 'ar' : ''"
                v-if="open">
                <li v-for="(page, index) in pages" :key="index" @click="$router.push(page.route)"
            ...
            <!-- Right side icons -->
            <div class="sm:w-[20%] w-[40%] media">
                <ul class="flex md:justify-end justify-evenly md:gap-2 lg:gap-4 items-center">
                    <!-- Color picker -->
                    <li id="colors"
                        class="glowing-circle w-[35px] h-[35px] rounded-full flex justify-center items-center"
                        @click="colors = !colors" v-tooltip="'Change color theme'">
                    ...
                    <div v-if="colors" :class="currentLanguage == 'ar' ? 'ar' : ''"
                        ...
                        <span v-for="color in colorOptions" :key="color"
                            class="rounded-full w-[20px] h-[20px] cursor-pointer" :style="{ backgroundColor: color }"
                    ...
                    <li class="glowing-circle w-[35px] h-[35px] rounded-full flex justify-center items-center"
                        @click="changelang" v-tooltip="'Switch language'">
                    ...
                    <!-- dashboard -->
                    <li class="glowing-circle w-[35px] h-[35px] rounded-full flex justify-center items-center" v-tooltip="'Dashboard'" @click="godashboard()">
                        <svg fill="#000000" viewBox="0 0 24 24" id="dashboard" data-name="Flat Line" xmlns="http://www.w3.org/2000/svg" class="icon flat-line"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path id="secondary" d="M9,12H4a1,1,0,0,1-1-1V4A1,1,0,0,1,4,3H9a1,1,0,0,1,1,1v7A1,1,0,0,1,9,12Zm12,8V13a1,1,0,0,0-1-1H15a1,1,0,0,0-1,1v7a1,1,0,0,0,1,1h5A1,1,0,0,0,21,20Z" style="fill: #33755f; stroke-width: 2;"></path><path id="primary" d="M21,7V4a1,1,0,0,0-1-1H15a1,1,0,0,0-1,1V7a1,1,0,0,0,1,1h5A1,1,0,0,0,21,7ZM10,20V17a1,1,0,0,0-1-1H4a1,1,0,0,0-1,1v3a1,1,0,0,0,1,1H9A1,1,0,0,0,10,20ZM9,12H4a1,1,0,0,1-1-1V4A1,1,0,0,1,4,3H9a1,1,0,0,1,1,1v7A1,1,0,0,1,9,12Zm12,8V13a1,1,0,0,0-1-1H15a1,1,0,0,0-1,1v7a1,1,0,0,0,1,1h5A1,1,0,0,0,21,20Z" style="fill: none; stroke: #ffffff; stroke-linecap: round; stroke-linejoin: round; stroke-width: 2;"></path></g></svg>
                    </li>
                </ul>
            </div>
        </nav>
...
const pages = [
    { name: 'home', route: '/', tooltip: 'Home page' },
    { name: 'skills', route: '/skills', tooltip: 'My skills' },
    // { name: 'projects', route: '/projects', tooltip: 'My projects' },
    { name: 'about', route: '/about', tooltip: 'About me' },
    { name: 'Contact', route: '/Contactme', tooltip: 'Contact me' }
];
...
onMounted(() => {
    ...
    document.documentElement.style.setProperty('--mainColor', localStorage.getItem('color') || '#436074');
    document.body.style.direction = locale.value == 'ar' ? 'rtl' : 'ltr';
    open.value = window.innerWidth >= 420;
});
...
const godashboard = () => {
    if (localStorage.getItem('dashboard').length == 320) {
...
@media (max-width: 770px) {
    ...
    .logo {
        font-size: clamp(.8rem, 1.7vw, 1.7rem);
        width: 45%;
    }
}
